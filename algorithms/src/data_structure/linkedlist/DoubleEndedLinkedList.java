package data_structure.linkedlist;// A Double Ended LinkedList has a reference to
...
public class DoubleEndedLinkedList {
    ...
    private void insertInFirstPosition(String homeOwnerName, int houseNumber){
        ...
        // If no items in the list add the new Link to lastLink in the LinkedList
        if(isEmpty()) {
            lastLink = theNewLink;
        ...
        /**
         *  DOUBLY LINKED LIST
         *  Just like you can go forward in the list with next with a doubly linked list you can go backwards because
         *  it also has a previous as well as a next
         *
         *  Assign the reference to the previous firstLink and assign the new Link to firstLink in LinkedList
         */
    ...
    private void insertInLastPosition(String homeOwnerName, int houseNumber){
        ...
        // If empty put the new data_structure.linkedlist.Neighbor in first position
        if(isEmpty()){
            firstLink = theNewLink;
        } else {
            // Assign the last Neighbors next to the new data_structure.linkedlist.Neighbor
    ...
    private boolean insertAfterKey(String homeOwnerName, int houseNumber, int key){
        ...
        while(currentNeighbor.houseNumber != key){
            currentNeighbor = currentNeighbor.next; // Switch to the next data_structure.linkedlist.Neighbor
            // If we get to the last data_structure.linkedlist.Neighbor without a match leave the method
        ...
        // If the match was for the last data_structure.linkedlist.Neighbor in the list

        if(currentNeighbor == lastLink){
            // Assign the new data_structure.linkedlist.Neighbor as the last link
            theNewLink.next = null;
            lastLink = theNewLink;

        } else {
            // It didn't match for the last link So take next from the data_structure.linkedlist.Neighbor that was here previously and assign
            // theNewLink to the previous data_structure.linkedlist.Neighbor
    ...
    public static void main(String[] args) {
		...
		*/
        ...
        // Switch to the next data_structure.linkedlist.Neighbor
        neighbors.next();
        neighbors.currentNeighbor.display();
        neighbors.remove();
        neighbors.currentNeighbor.display();
    ...
    public void insertInOrder(String homeOwnerName, int houseNumber){
        ...
        // Holds he last data_structure.linkedlist.Neighbor searched so we can change its value for next if we input a new data_structure.linkedlist.Neighbor
        Neighbor previousNeighbor = null;
        Neighbor currentNeighbor = firstLink;

        // While there are still Neighbors and the new houseNumber is greater than the current focused houseNumber
        // Change the > to < for opposite sort
        while((currentNeighbor != null) && (houseNumber > currentNeighbor.houseNumber)){
            previousNeighbor = currentNeighbor;
            currentNeighbor = currentNeighbor.next; // Get the next data_structure.linkedlist.Neighbor
        }

        // We are still at the beginning of the list
        if(previousNeighbor == null){
            // Save new data_structure.linkedlist.Neighbor in the first position
            firstLink = theNewLink;
        } else {
            // Assign the new data_structure.linkedlist.Neighbor as the value for next
            previousNeighbor.next = theNewLink;
        }

        // Assign the value of next to the next data_structure.linkedlist.Neighbor
...
class NeighborIterator{
    ...
    Neighbor currentNeighbor; // The current focus data_structure.linkedlist.Neighbor
    Neighbor previousNeighbor; // The previous data_structure.linkedlist.Neighbor
