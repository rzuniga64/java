package data_structure;// A Double Ended LinkedList has a reference to
...
public class DoubleEndedLinkedList {
    ...
    private void insertInLastPosition(String homeOwnerName, int houseNumber){
        ...
        // If empty put the new data_structure.Neighbor in first position
        if(isEmpty()){
            firstLink = theNewLink;
        } else {
            // Assign the last Neighbors next to the new data_structure.Neighbor
    ...
    private boolean insertAfterKey(String homeOwnerName, int houseNumber, int key){
        ...
        while(currentNeighbor.houseNumber != key){
            currentNeighbor = currentNeighbor.next; // Switch to the next data_structure.Neighbor
            // If we get to the last data_structure.Neighbor without a match leave the method
        ...
        // If the match was for the last data_structure.Neighbor in the list

        if(currentNeighbor == lastLink){
            // Assign the new data_structure.Neighbor as the last link
            theNewLink.next = null;
            lastLink = theNewLink;

        } else {
            // It didn't match for the last link So take next from the data_structure.Neighbor that was here previously and assign
            // theNewLink to the previous data_structure.Neighbor
    ...
    public static void main(String[] args) {
		...
		*/
        ...
        // Switch to the next data_structure.Neighbor
        neighbors.next();
        neighbors.currentNeighbor.display();
        neighbors.remove();
        neighbors.currentNeighbor.display();
    ...
    public void insertInOrder(String homeOwnerName, int houseNumber){
        ...
        // Holds he last data_structure.Neighbor searched so we can change its value for next if we input a new data_structure.Neighbor
        ...
        while((currentNeighbor != null) && (houseNumber > currentNeighbor.houseNumber)){
            ...
            currentNeighbor = currentNeighbor.next; // Get the next data_structure.Neighbor
        }

        // We are still at the beginning of the list
        if(previousNeighbor == null){
            // Save new data_structure.Neighbor in the first position
            firstLink = theNewLink;
        } else {
            // Assign the new data_structure.Neighbor as the value for next
            previousNeighbor.next = theNewLink;
        }

        // Assign the value of next to the next data_structure.Neighbor
...
class NeighborIterator{
    ...
    Neighbor currentNeighbor; // The current focus data_structure.Neighbor
    Neighbor previousNeighbor; // The previous data_structure.Neighbor
