...
import java.util.*;
     ...
     *  similarly to synchronized methods except now multiple threads can run
     *  the same method at the same time. But if one thread enters the
     *  synchronized code block the other threads will have to wait until the
     *  first thread has released the lock passed in and exits the code block
     *  so that it can run the synchronized code block.  Since we are using two
     *  different locks one thread can run StageOne while another thread can run
